...
const getAverageReliabilityData = function(data) {
    const averageReliabilityData = [];

    // Total number of items (moments at which a measure was conducted) in the dataset
    const indexes = _.range(data.values().next().value.length);
    // Total number of components
    const totalComponents = data.size;
    ...
    _.forEach(indexes, i => {
        ...
        averageReliabilityData.push({
            checked_date: data.values().next().value[i].checked_date,
...
const addGraphs = function(project, displayName, source) {
    ...
    axios.all([
        ...
        const averageReliabilityData = getAverageReliabilityData(data);
