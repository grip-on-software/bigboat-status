...
let addGraphs = function(project, source) {
    ...
    axios.all([
        ...
        let updateRange = (duration, element, buttons) => {
            // Set this duration as active
            d3.select("#range .is-active").classed('is-active', false);
            d3.select(buttons[element]).classed('is-active', true);

            averageReliabilityGraph.setDomain(typeof duration === "object" ?
                moment.duration(...duration) : undefined
            );
        };
        d3.select('#range ul')
            .selectAll('li')
            .on("click", updateRange)
            .data(['full', [1, 'month'], [1, 'week']])
            .enter()
            .append('li')
            .on("click", updateRange)
            .classed('is-active', d => d === 'full')
            .append('a')
            .text(d => typeof d === "object" ? d[1] : d);
        d3.select('#range ul li.is-active').each(updateRange);
