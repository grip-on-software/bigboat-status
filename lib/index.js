...
let addGraphs = function(project) {
    ...
    axios.get(`data/bigboat_status/${project}.json`).then(response => {
        ...
        const averageReliabilityGraph = new graph(averageReliabilityData, {
            element: d3.select('svg#average-reliability')
        });

        // Create a graph for each component
        _.forEach(data, function (componentData) {
            ...
            const componentGraph = new graph(componentData.values, {
                element: svg
            });
        });
    ...
    d3.select('#navigation ul')
        ...
        .on("click", (project, element, buttons) => {
            ...
            if (project != currentProject) {
                ...
                d3.select('svg#average-reliability *').remove();
