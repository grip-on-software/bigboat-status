...
class graph {
    ...
    create() {
        ...
        const valueUnit = new unit(this.config);
        if (this.componentHasValues) {
            ...
            g.append('g')
                ...
                .text(this.locales.attribute('axes', valueUnit.key));
    ...
    mousemove() {
        ...
        if (this.focusData.value) {
            const unit = vsprintf(this.locales.attribute('units', valueUnit.key), [valueUnit.format(this.focusData.value)]);
            tooltipText.append('tspan')
                .attr('x', 15)
                .attr('dy', '1.2em')
                .text(unit);
