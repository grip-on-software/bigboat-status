...
class graph {
    ...
    constructor(data, dispatch, configuration = {}, locales=undefined) {
        this.data = data;
    ...
    create() {
        ...
        const yValueStartDomain = [0, d3.max(this.data, d => d.max ? d.max : d.value)];

        this.x.domain(xStartDomain);
        this.yOk.domain([0, 1]);
        this.yValue.domain(yValueStartDomain);
        ...
        if (this.componentHasValues) {
            ...
            this.yValueAxis.tickFormat(unitFormat);
            ...
            g.append('g')
                ...
                .text(this.locales.attribute('axes', unitKey));
        ...
        let mousemove = () => {
            ...
            if (this.focusData.value) {
                const unit = vsprintf(this.locales.attribute('units', unitKey), [unitFormat(this.focusData.value)]);
                tooltipText.append('tspan')
                    .attr('x', 15)
                    .attr('dy', '1.2em')
                    .text(unit);
    ...
    zoom() {
        ...
        if (this.yValueAxis) {
            ...
            const yValueDomain = [0, d3.max(this.data, d => d.checked_date >= xDomain[0] && d.checked_date <= xDomain[1] ? (d.max ? d.max : d.value) : 0)];
